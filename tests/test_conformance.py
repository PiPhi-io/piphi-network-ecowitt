...
from piphi_network_ecowitt.ecowitt import EcowittSnapshot
from piphi_network_ecowitt.main import app
from piphi_network_ecowitt.state import ecowitt_service, registry

FIXTURES = json.loads(
    (Path(__file__).parent / "fixtures" / "contract-conformance.json").read_text()
)


@pytest.mark.anyio
async def test_runtime_conforms_to_shared_contract_fixtures(monkeypatch) -> None:
    monkeypatch.setattr(ecowitt_service, "client_factory", lambda _config: FakeClient())
    transport = httpx.ASGITransport(app=app)
    try:
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            for fixture in FIXTURES["cases"]:
                response = await client.request(
                    fixture["method"], fixture["path"], json=fixture.get("body")
                )
...
class FakeClient:
    async def read_snapshot(self) -> EcowittSnapshot:
        return EcowittSnapshot(
            gateway_metrics={"connected": True, "temperature_c": 21.5},
            gateway_units={"connected": "bool", "temperature_c": "C"},
            sensors=(),
            raw_sections=("common_list",),
        )
...
def _assert_required_any_keys(
    body: dict[str, Any], groups: list[list[str]], fixture_id: str
) -> None:
    for group in groups:
